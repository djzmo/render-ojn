...
RenderResult render_chart(const emscripten::val& ojn_bytes, const emscripten::val& ojm_bytes, int difficulty,
    ...
    output::Tags tags{header.title, header.artist, header.song_id, format::genre_name(header.genre, diagnostics),
                      std::string("Generated by RenderOJN v") + app::version::kVersion + "\n" +
                          app::version::kProjectUrl};
    // The chart's cover rides along in the tags, exactly as the CLI does by default.
