...
RenderResult render_chart(const emscripten::val& ojn_bytes, const emscripten::val& ojm_bytes, int difficulty,
    ...
    const auto encoded = output::encode_to_buffer(
        ...
        [&](const auto& consume) {
            // Always Quick scheduling: it places every trigger at its exact frame,
            // and it is never wall-clock paced (that only makes sense for live
            // playback, which the browser build does not offer).
